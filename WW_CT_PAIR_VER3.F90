...
integer,parameter:: mxhb=500000,mxtype=10,mxw=17000
real,dimension(mxatom):: x,y,z
real(kind=8),dimension(0:mxframe):: ct,av,avsq
...
call HBOND(nu,nhb,ni,nf,nsit,rm,mxhb,x,y,z,al,rl,rh,uu,h)
enddo
enddo
!------------------------------------------------------------------------------
ct(:)=0.d0
...
if(nu > mxframe)nu=nu-1
do i=1,nhb
do j=1,nu-1
if(h(i,j) == 0)cycle
do k=j+1,nu
m=k-j
ct(m)=ct(m)+h(i,j)*h(i,k)
n(m)=n(m)+1.0
enddo
enddo
enddo
...
!------------------------------------------------------------------------------
ncorr=nu/2    !for better statistical averaging
do i=0,ncorr
if(n(i) == 0)n(i)=1
if(i == 0)then
ct(i)=1.0
...
!------------------------------------------------------------------------------
subroutine HBOND(nu,nhb,ni,nf,ns,rm,mxhb,x,y,z,al,rl,rh,uu,h)
